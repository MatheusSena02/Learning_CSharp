...
O Git é um **sistema de controle de versão distribuído (VSC)** e de código aberto, criado por Linus Torvalds em 2005. Diferente de sistemas mais antigos como o SVN, onde o histórico de alterações é centralizado, o Git permite que cada desenvolvedor tenha uma cópia completa do histórico do repositório em sua máquina local.

Essa abordagem distribuída torna operações diárias como `commit`, `blame`, `diff`, `merge` e `log`  muito mais rápidas, já que não dependem de uma conexão constante com um servidor central.

O Git também se destaca pelo seu poderoso suporte a ramificação (branching), mesclagem (merging) e pela capacidade de reescrever o histórico de um repositório. Isso possibilitou o surgimento de fluxos de trabalho colaborativos eficientes, como o uso de pull requests. As pull requests são uma ferramenta popular que permite que equipes trabalhem em ramificações isoladas e revisem o código umas das outras antes de mesclar as alterações para o projeto principal.
...
- **style:** Mudanças que não afetam o funcionamento do código (espaços, formatação, vírgulas)  
  *Exemplo:* `style: ajustar indentação do código`

- **refactor:** Alterações no código que não mudam a funcionalidade, mas melhoram a estrutura  
  *Exemplo:* `refactor: otimizar função de cálculo`

- **perf:** Alterações para melhorar desempenho  
  *Exemplo:* `perf: reduzir tempo de carregamento da página inicial`

- **test:** Adição ou modificação de testes  
  *Exemplo:* `test: criar testes unitários para serviço de autenticação`

- **chore:** Alterações em tarefas de build, dependências ou configurações  
...
- **revert:** Reverter um commit anterior  
  *Exemplo:* `revert: voltar alteração que causou erro em produção`

- **hotfix:** Correção rápida de problema crítico em produção  
  *Exemplo:* `hotfix: corrigir falha de autenticação em produção`
...
### Submódulos, subárvores e submódulos avançados
...
### Ganchos e automação, e ferramentas de comparação e mesclagem
...
### Problemas de bissecção, depuração e desempenho
